...
def process_func_single(example, tokenizer):
    ...
    try:
        # 构建对话结构
        question = f"{example["question"]} /think" if example["type"] == "reason" else f"{example["question"]} /no_think"
        messages = [
        ...
        # 生成响应内容
        response_text = f"<think>{example['cot']}</think>\n{example['answer']}" if example["type"] == "reason" else f"<think>\n\n</think>\n{example['answer']}"
...
def process_func_batch(examples, tokenizer, max_length=2048):
    ...
    for question, cot, answer, type in zip(examples["question"], examples["cot"], examples["answer"], examples["type"]):
        # 构建对话结构
        question = f"{question} /think" if type == "reason" else f"{question} /no_think"
...
swanlab_callback = SwanLabCallback(
    project="Qwen3-1.7B",
    experiment_name="Qwen3-1.7B-finetune",
    description="使用通义千问Qwen3-1.7B模型在FreedomIntelligence/medical-o1-reasoning-SFT和BAAI/IndustryInstruction_Health-Medicine数据集上微调。",
    config={
...
args = TrainingArguments(
    output_dir="./lora_train_off_1_7b",
...
trainer.train()
trainer.save_model('./lora_train_off_1_7b')
trainer.save_state()
