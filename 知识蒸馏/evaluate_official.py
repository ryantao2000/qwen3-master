...
# 1. 加载基础模型
base_model_name = "merged_model_finetune_distillation"
        ...
        for lang, group in lang_groups.items():
            ...
            try:
                P, R, F1 = score(
                    g_preds, 
                    g_refs,
                    model_type = "/root/autodl-tmp/metric/bert-base-chinese" if lang == 'zh' else "/root/autodl-tmp/metric/roberta-large",
...
def main():
    ...
    # 保存完整结果
    with open("evaluation_1_7b_on_policy.json", "w") as f:
        json.dump({
            "test_data": test_data,
            "results": results
